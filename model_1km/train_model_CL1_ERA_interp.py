...
dtype_dict = {col: np.float32 for col in ['tas', 'T_TARGET', "LC_1_perc", "LC_2_perc", "LC_3_perc", "LC_4_perc", "LC_5_perc", 
    "LC_6_perc", "LC_7_perc", "LC_8_perc", "LC_9_perc", "LC_10_perc", "LC_11_perc", "LC_12_perc", 
    "LC_13_perc", "LC_14_perc", "LC_15_perc", 'IMPERV', 'HEIGHT', 'COAST', 'ELEV', 'POP', 'RH', 
    'SP', 'PRECIP', 'wind_speed', 'TCC', 'CAPE', 'BLH', 'SSR','SOLAR_ELEV', 'DECL']}
    
file_path = '/data/gent/vo/000/gvo00041/Kwint_personal/UrbClim_Emulator/ownscripts/model_1km/train_val_ERA_interp/CLUSTER1_TEST_1km.csv'

# Step 1: Load without applying dtype to catch potential string issues
val = pd.read_csv(
    file_path,
    usecols=list(dtype_dict.keys()) + ['city'],
    dtype={'city': 'category'},
    low_memory=False,
    on_bad_lines='skip'
)

# Step 2: Drop rows where 'tas' column has the string 'tas'
val = val[val['tas'].astype(str) != 'tas']

# Step 3: Convert columns to float32 as intended
for col in dtype_dict:
    val[col] = pd.to_numeric(val[col], errors='coerce').astype(np.float32)

file_path = '/data/gent/vo/000/gvo00041/Kwint_personal/UrbClim_Emulator/ownscripts/model_1km/train_val_ERA_interp/CLUSTER1_TRAIN_1km.csv'
...
# Step 2: Drop rows where 'tas' column has the string 'tas'
train = train[train['tas'].astype(str) != 'tas']
...
val['T2M_difference'] = (val['T_TARGET'] - val['T2M']).astype(np.float32)
val['T2M'] = (val['T2M'] - 273.15).astype(np.float32)
train['T2M_difference'] = (train['T_TARGET'] - train['T2M']).astype(np.float32)
train['T2M'] = (train['T2M'] - 273.15).astype(np.float32)
...
with open('run_time_CL1_ERA_interp.txt', 'a') as f:
...
y_train = train['T2M_difference'].astype(np.float32)
train = train[["LC_1_perc", "LC_2_perc", "LC_3_perc", "LC_4_perc", "LC_5_perc", 
                                                                          ...
                                                                          "LC_11_perc", "LC_12_perc", "LC_13_perc", "LC_14_perc", "LC_15_perc",
                                                                            'IMPERV', 'HEIGHT', 'COAST', 'ELEV', 'POP', 'RH', 'SP', 'PRECIP','T2M', 'WS', 'TCC',  'CAPE', 'BLH', 'SSR','SOLAR_ELEV', 'DECL']].astype(np.float32)

y_val = val['T2M_difference'].astype(np.float32)
...
joblib.dump(rf, "/data/gent/vo/000/gvo00041/Kwint_personal/UrbClim_Emulator/ownscripts/model_1km/models/model_1km_CL1_ERA_interp.joblib")
