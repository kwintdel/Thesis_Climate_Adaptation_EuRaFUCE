...
output_dir = '/data/gent/vo/000/gvo00041/Kwint_personal/UrbClim_Emulator/ownscripts/raster_1km/ERA_solar_new_sep'
...
for era5_file in os.listdir(era_dir):
    if era5_file.endswith(".nc"):
        ...
        ds = ds.rename({"longitude": "x", "latitude": "y"})  # Rename to match city files

        date = era5_file[5:-3]

        city_file = "Bruges.nc"
        # Loop over all city files in the city_dir
        for i in [1,2]: #city_file in os.listdir(city_dir):
            if i == 2:
                continue
            ...
            clipped_ds = ds.rio.reproject_match(city, resampling=Resampling.average)
